// Baixa as splashes _0 dos campeões que faltam do Data Dragon.
// Run: node scripts/download-missing-splashes.mjs
import { writeFile, mkdir } from "node:fs/promises";
import { existsSync } from "node:fs";
import path from "node:path";

const ROOT = path.resolve(path.join(import.meta.dirname, ".."));
const OUT_DIR = path.join(ROOT, "campeões", "img", "champion", "splash");
...
for (const champ of MISSING) {
  ...
  const res = await fetch(SPLASH_URL(champ));
  if (!res.ok) { console.log(`✗ ${champ} (${res.status})`); miss++; continue; }
